...
(() => {
  ...
  // src/physics/replica/nape-core.ts
  var TYPE_STATIC = 1;
  var TYPE_DYNAMIC = 2;
  var TYPE_KINEMATIC = 3;
  var STATIC_MASS = 179e306;
  var _fisrF32 = new Float32Array(1);
  var _fisrI32 = new Int32Array(_fisrF32.buffer);
  ...
  function combineGeoMean(a, b) {
    const x = a * b;
    if (x === 0) return 0;
    return 1 / fastInvSqrt(x);
  }
  function pairKey(h1, h2) {
    return h1 < h2 ? `${h1}-${h2}` : `${h2}-${h1}`;
  }
  function makeFilter(colCat, colMask, isSensor) {
    if (isSensor) {
  ...
  var NapeReplica = class {
    constructor(gravityPxY) {
      // Space (zpp_nape.space.ZPP_Space)
      __publicField(this, "gravityx", 0);
      __publicField(this, "gravityy", 0);
      __publicField(this, "global_lin_drag", 0.015);
      // ZPP_Space.as:205
      __publicField(this, "global_ang_drag", 0.015);
      // ZPP_Space.as:206
      __publicField(this, "bodies", /* @__PURE__ */ new Map());
      __publicField(this, "live", []);
      __publicField(this, "nextHandle", 1);
      __publicField(this, "nextShapeId", 1);
      // [E2] per-shape id, so arbiters key per shape-PAIR
      ...
      __publicField(this, "arbiters", /* @__PURE__ */ new Map());
      __publicField(this, "stamp", 0);
      ...
      __publicField(this, "activeSenPairs", /* @__PURE__ */ new Set());
      this.gravityx = 0;
    ...
    // --- body construction --------------------------------------------------
    createBody(isStatic, xPx, yPx, rotDeg, _linDamp, _angDamp) {
    ...
    addCircle(h, posX, posY, radius, density, friction, rolling, elasticity, colCat, colMask, isSensor) {
      ...
      if (b == null) return;
      b.shapes.push(__spreadProps(__spreadValues({
        kind: "circle",
        localx: posX,
        localy: posY,
        radius,
        density: density / 1e3,
        // Material stores density/1000 (Material.as:51)
        friction,
        rolling,
        elasticity,
        area: 0,
    ...
    addPolygon(h, vertsFlat, density, friction, rolling, elasticity, colCat, colMask, isSensor) {
      const b = this.bodies.get(h);
      if (b == null) return;
      b.shapes.push(__spreadProps(__spreadValues({
        kind: "polygon",
    ...
    validateMassProps(b) {
      ...
      if (b.type === TYPE_DYNAMIC) {
        ...
        b.sinertia = b.iinertia = 1 / b.inertia;
      } else {
    ...
    doForests(dt) {
      ...
      for (const arb of this.arbiters.values()) {
        ...
        for (const c of arb.contacts) if (c.stamp === this.stamp) {
          hasContact = true;
          break;
        }
    ...
    updatePos(dt) {
      for (const cur of this.live) {
        ...
        if (cur.type !== TYPE_DYNAMIC) {
          ...
          const fast = (cur.velx * cur.velx + cur.vely * cur.vely) * dt * dt > sr * sr || cur.angvel * cur.angvel * dt * dt > 5e-3 * 5e-3;
    ...
    addPivotJoint(hA, hB, a1x, a1y, a2x, a2y) {
    ...
    addDistanceJoint(hA, hB, a1x, a1y, a2x, a2y, jointMin, jointMax) {
      const b1 = this.bodies.get(hA);
      const b2 = this.bodies.get(hB);
      if (b1 == null || b2 == null) return;
      this.constraints.push({
    ...
    addAngleJoint(hA, hB, jointMin, jointMax, ratio = 1) {
      const b1 = this.bodies.get(hA);
      const b2 = this.bodies.get(hB);
      if (b1 == null || b2 == null) return;
      this.constraints.push({
    ...
    addMotorJoint(hA, hB, rate = 0, ratio = 1) {
      const b1 = this.bodies.get(hA);
      const b2 = this.bodies.get(hB);
      if (b1 == null || b2 == null) return;
      this.constraints.push({
    ...
    addPartner(hA, hB) {
      ...
      if (la == null) {
        la = [];
        this.jointPartners.set(hA, la);
      }
      la.push(hB);
      let lb = this.jointPartners.get(hB);
      if (lb == null) {
        lb = [];
        this.jointPartners.set(hB, lb);
      }
      lb.push(hA);
    }
    ...
    jointDist(hA, hB, x0, y0, x1, y1, distLimit, _soft, _freq) {
      const b1 = this.bodies.get(hA);
      const b2 = this.bodies.get(hB);
      if (b1 == null || b2 == null || b1 === b2) return;
      this.addPartner(hA, hB);
    ...
    raycastDown(xPx, fromYPx, maxDist, colCat) {
      let bestY = Infinity;
      ...
      for (const b of this.live) {
        for (const s of b.shapes) {
          ...
          if (s.kind === "circle") {
            ...
            const y = wc.y - Math.sqrt(s.radius * s.radius - ddx * ddx);
    ...
    // --- lifecycle ------------------------------------------------------------
    setTransform(h, xPx, yPx, rotDeg) {
      const b = this.bodies.get(h);
      if (b == null || b.type === TYPE_STATIC) return;
      b.posx = xPx;
      b.posy = yPx;
    ...
    setShapeCollisionMask(h, shapeIdx, mask) {
      const b = this.bodies.get(h);
      if (b == null) return;
    ...
    setBodyCollisionAboveTop(h, topThresholdPx, enabled) {
      const b = this.bodies.get(h);
      if (b == null) return;
      for (let i = 0; i < b.shapes.length; i++) {
        const s = b.shapes[i];
        const aabb = this.shapeAABB(h, i);
        const topPx = b.posy - aabb[1];
    ...
    circleVertexContact(vx, vy, wcx, wcy, r) {
      const lx = vx - wcx;
      const ly = vy - wcy;
    ...
    circlePolyManifold(hc, hp) {
      ...
      return { nx: bgnx, ny: bgny, penetration: -best };
    }
    ...
    polyWorldGeom(b, s) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        edges.push({
          ...
          gp1x,
          gp1y,
          tp0: gp0y * gnx - gp0x * gny,
          // ZPP_Polygon.as:198
          tp1: gp1y * gnx - gp1x * gny,
          lnx,
          // edge normal in the polygon's LOCAL frame (for iteratePos reconstruct)
          lny,
          lproj
        });
      }
    ...
    // ======================================================================== //
    distanceQuery(bA, sA, bB, sB, maxDist = 1e100) {
      if (sA.kind === "circle" && sB.kind === "circle") {
        ...
        const d3 = dist - (sA.radius + sB.radius);
        let p3x = 0;
        let p3y = 0;
        let p4x = 0;
        let p4y = 0;
        ...
        if (d3 < maxDist) {
          ...
          p3x = w1.x + nx2 * sA.radius;
          p3y = w1.y + ny2 * sA.radius;
          p4x = w2.x + nx2 * -sB.radius;
          p4y = w2.y + ny2 * -sB.radius;
          p5x = nx2;
          p5y = ny2;
        }
        return { d: d3, p3x, p3y, p4x, p4y, p5x, p5y };
      }
      let swapped = false;
      let pB;
      let pS;
    ...
    distancePolyPoly(bA, sA, bB, sB, maxDist) {
      const w1 = this.polyWorldGeom(bA, sA);
      const w2 = this.polyWorldGeom(bB, sB);
    ...
    staticSweep(ev, dt) {
      ...
      if (sa < 0) sa = -sa;
      const angBound = ev.ms.sweepCoef * sa;
      let t = 0;
      let iter = 0;
      for (; ; ) {
    ...
    continuousCollisions(dt) {
      ...
      while (t < 1 && events.length > 0) {
        ...
        if (ev.ms.kind === "circle" && ev.ss.kind === "circle") {
          ...
          arb = this.narrowPolyPoly(ev.stat, ev.ss, mv, ev.ms);
        }
        if (arb != null) {
          this.prestep(dt, arb);
    ...
    narrowCircleCircle(bA, sA, bB, sB) {
      ...
      if (d2 < 1e-8 * 1e-8) {
        ...
        const interp = 0.5 + (sA.radius - 0.5 * rsum) * invSqrt;
        px = wa.x + dx * interp;
        py = wa.y + dy * interp;
        nx = dx * invSqrt;
        ny = dy * invSqrt;
    ...
    polyPolyContact(arb, hash, clx, cly, d, ref, incBody) {
    ...
    narrowCirclePoly(cBody, pBody, sc, sp) {
      ...
      if (t <= tp0 || t >= tp1) {
        ...
        if (d2 < 1e-8) {
          ...
          const invSqrt = fastInvSqrt(d2);
          const dist = invSqrt < 1e-8 ? 1e100 : 1 / invSqrt;
          const interp = 0.5 + (r - 0.5 * r) * invSqrt;
          px = wcx + lx * interp;
          py = wcy + ly * interp;
          nx = lx * invSqrt;
          ny = ly * invSqrt;
    ...
    prestep(dt, only) {
      for (const arb of only ? [only] : this.arbiters.values()) {
        ...
        if (arb.hc2) {
          ...
          if (arb.kMassa * arb.kMassa < 4e5 * (arb.kMassa * arb.kMassc - arb.kMassb * arb.kMassb)) {
            arb.Ka = arb.kMassa;
            arb.Kb = arb.kMassb;
            arb.Kc = arb.kMassc;
            ...
            if (det !== det) {
              ...
              if (arb.kMassa !== 0) arb.kMassa = 1 / arb.kMassa;
              else arb.kMassa = 0;
              if (arb.kMassc !== 0) arb.kMassc = 1 / arb.kMassc;
              else arb.kMassc = 0;
              arb.kMassb = 0;
            } else {
              det = 1 / det;
      ...
      if (only == null)
        for (const c of this.constraints) {
          if (c.b1.sleeping && c.b2.sleeping) continue;
        ...
        for (const arb of ordered) {
          ...
          if (sep < 0 || sep2 < 0) {
            ...
            if (c2 !== null) {
              ...
              const cr11 = nly * a1x - nlx * a1y;
              const cr12 = nly * a2x - nlx * a2y;
              const cr21 = nly * a1px - nlx * a1py;
              const cr22 = nly * a2px - nlx * a2py;
              const m = b1.smass + b2.smass;
              ...
              const bs1 = sep * arb.biasCoef;
              const bs2 = sep2 * arb.biasCoef;
              let l39 = -bs1;
              let l40 = -bs2;
              let apply = false;
    ...
    distanceApplyImpulsePos(c) {
      ...
      if (err * err > 6) {
        ...
        if (m > 1e-8) {
          m = 0.75 / m;
          const lambda2 = -err * m;
          if (c.equal || lambda2 < 0) {
            let s = lambda2 * b1.imass;
            b1.posx -= nx * s;
            b1.posy -= ny * s;
            s = lambda2 * b2.imass;
            b2.posx += nx * s;
