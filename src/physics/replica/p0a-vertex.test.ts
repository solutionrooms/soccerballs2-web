// P0a gate (bit-exact) — circle-VERTEX contact (ptype 2). The narrowphase used to
// THROW on a ball nearest a polygon corner; terrain is triangulated, so that's the
// common case. Golden from harness-p0a.as: a ball balanced on a box's top-right
// corner (centre directly above it → vertical normal), settling via the discrete
...
const lines: string[] = JSON.parse(
  readFileSync(fileURLToPath(new URL('./original-goldens/p0a-vertex.json', import.meta.url)), 'utf8'),
).lines;
...
const frames = lines
  .filter((l) => l.startsWith('[P0A] ') && !l.includes('DONE'))
  .map((l) => {
    const p = l.split(/\s+/); // [P0A] i x y rot vx vy angvel
    return { i: Number(p[1]), x: p[2], y: p[3], rot: p[4], vx: p[5], vy: p[6], angvel: p[7] };
  });

describe('P0a — circle-vertex contact (ball on a corner) vs ORIGINAL Nape AS3', () => {
  it('ball state matches the original bit-for-bit at every one of 60 steps', () => {
    ...
    const ball = w.createBody(false, 300, 270, 0, 0, 0); // centre above the corner (300,280)
