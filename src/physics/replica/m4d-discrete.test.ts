// Milestone M4 gate — the DISCRETE contact solver, vs the ORIGINAL Nape AS3.
// Golden from harness-m4d.as: a dynamic circle placed already penetrating a
// STATIC polygon floor (2px), at rest, settling under gravity to the resting
// separation. Per-step velocity-driven motion stays below Nape's continuous-
// collision threshold, so this isolates the discrete pipeline (narrowphase →
// prestep → warmStart → iterateVel → iteratePos) from CCD. We assert the ball's
// full state every step, bit-for-bit, reporting the FIRST diverging step/field.
...
const lines: string[] = JSON.parse(
  readFileSync(fileURLToPath(new URL('./original-goldens/m4d-discrete.json', import.meta.url)), 'utf8'),
).lines;
...
const frames = lines
  .filter((l) => l.startsWith('[M4D] ') && !l.includes('DONE'))
  .map((l) => {
    const p = l.split(/\s+/); // [M4D] i x y rot vx vy angvel
    return { i: Number(p[1]), x: p[2], y: p[3], rot: p[4], vx: p[5], vy: p[6], angvel: p[7] };
  });

describe('M4 — DISCRETE contact solver (penetrating rest) vs ORIGINAL Nape AS3', () => {
  it('ball state matches the original bit-for-bit at every one of 60 steps', () => {
    const w = new NapeReplica(1000);
    const MAT = [1.0, 0.5, 0.1, 0.0, 1, 0xffff, false] as const; // density,fric,roll,elast,cat,mask,sensor

    const floor = w.createBody(true, 200, 300, 0, 0, 0);
    w.addPolygon(floor, [-200, -20, 200, -20, 200, 20, -200, 20], ...MAT);
    w.finalizeBody(floor, false);

    const ball = w.createBody(false, 200, 270, 0, 0, 0); // start penetrating 2px
